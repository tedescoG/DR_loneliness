...
treated = subset(inc_dec, remote_contact == 1)

weighted_design = svydesign(ids = ~1, weights = ~iptw, data = inc_dec)
...
treated = subset(inc_mix, remote_contact == 1)

weighted_design = svydesign(ids = ~1, weights = ~iptw, data = inc_mix)
### Model 1: Outcome ~ Treatment ####
# Fitting weighted regression
